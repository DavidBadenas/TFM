dat <- read.table("logistic_results_prostate.assoc_2.logistic", header=T)
dat$BETA <- log(dat$OR)
write.table(dat, "prostate.Transformed", quote=F, row.names=F)
q()

singularity run -H $PWD:/mnt/bioinfnas/bioinformatics/test/david/GWAS_prostate_con_la_mitad/ plink19.sif /plink \
    --bfile imputed_prostate_13 \
    --clump-p1 1 \
    --clump-r2 0.1 \
    --clump-kb 250 \
    --clump prostate.Transformed \
    --clump-snp-field SNP \
    --clump-field P \
    --out prostate
	
	
awk 'NR!=1{print $3}' prostate.clumped >  prostate.valid.snp
awk '{print $2,$9}' logistic_results_prostate.assoc_2.logistic > SNP.pvalue
...
singularity run -H $PWD:/mnt/bioinfnas/bioinformatics/test/david/GWAS_prostate_con_la_mitad/ plink19.sif /plink \
    --bfile imputed_prostate_13 \
    --score prostate.Transformed 2 4 10 header \
    --q-score-range range_list SNP.pvalue \
    --extract prostate.valid.snp \
    --out prostate
...
phenotype <- read.table("prostate_phenotype.txt", header=T)
...
for(i in p.threshold){
    pheno.prs <- merge(pheno, 
                        read.table(paste0("prostate.",i,".profile"), header=T)[,c("FID","IID", "SCORE")],
                        by=c("FID", "IID"))
...
# save the plot
ggsave("prostate.height.bar.png", height = 7, width = 7)
q() # exit R
